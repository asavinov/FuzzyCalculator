...
#define MAXDIGITS 9 // defines a maxium of digits in the buffer

char DigBuffer[ MAXDIGITS + 5 ] = "0"; // a buffer of numbers

short nHscrollInc,
	nHscrollPos = 0,
	BuffPointer = 0; // a pointer to current position in the buffer
double FirstRegister = 0,
	SecondRegister   = 0;

double SParameter = 1; // a parameter using while computing an operation

char CurrentOp = ' '; // contains a current operation

char szAppName[] = "FuzzyCalculator"; // an application name
...
FARPROC lpfnAboutDlgProc; // a long pointer to a dialog procedure
...
// Function prototypes
//
LRESULT CALLBACK FcalcWndProc ( HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam );
...
BOOL FAR PASCAL _export AboutDlgProc ( HWND hDlg, WORD message, WORD wParam, LONG lParam )
{
	...
	case WM_COMMAND:
		switch (wParam)
		{
			case IDOK: // Button "OK" was pushed.
				...
				if (( szBuffer[0] == '0' ) & ( szBuffer[1] == 0 ))
					...
					{
						MessageBox ( hDlg, "Incorrect S-parameter!", "Fuzzy Calculator", MB_OK | MB_ICONEXCLAMATION );
			...
			case IDCANCEL: // Button "Cancel" was pushed.
				EndDialog ( hDlg, 0 );
				return TRUE;
		}
		break;
...
void CalcIt ( void )
{
	...
	{
	case '+': 
		FirstRegister = FuzzySum ( FirstRegister, SecondRegister ); break;
	case '-': 
		FirstRegister = FuzzySub ( FirstRegister, SecondRegister ); break;
	case '*': 
		FirstRegister = FuzzyMult ( FirstRegister, SecondRegister ); break;
	case '/': 
		FirstRegister = FuzzyDiv ( FirstRegister, SecondRegister ); break;
	case ' ': 
		FirstRegister = atof ( DigBuffer ); break;
	}
...
void DrawButton ( LPDRAWITEMSTRUCT pDrawStruct )
{
	...
	if ( pDrawStruct->itemAction & (ODA_DRAWENTIRE | ODA_SELECT ))
	{
		if ( !(pDrawStruct->itemState & ODS_SELECTED ))
			switch ( pDrawStruct->CtlID ) // Draw in normal state.
			{
			...
			switch ( pDrawStruct->CtlID ) // Draw in pressed state.
			{
		...
		BitBlt ( pDrawStruct->hDC, pDrawStruct->rcItem.left, pDrawStruct->rcItem.top, 100, 100, hdcCompat, 0, 0, SRCCOPY );
		DeleteDC ( hdcCompat );
	}
}
...
LRESULT CALLBACK FcalcWndProc ( HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam )
{
	...
	case WM_SYSCOMMAND:
		...
		if ( wParam == IDM_ABOUT ) // Create ABOUTDIALOG.
		{
	...
	case WM_HSCROLL:
		...
		switch (wParam) // Service messages of SCROLLBAR.
		{
